        ...
        private static async Task OwnerReplying(Update update)
        {
            ...
            if (Vars.CurrentConf.EnableMsgLink)
            {
                ...
                Vars.CurrentConf.MessageLinks.Add(
                    new Conf.MessageIDLink(update.Message.ReplyToMessage.ForwardFrom)
                    { OwnerSessionMessageID = update.Message.MessageId, UserSessionMessageID = forwarded.MessageId, IsFromOwner = true }
                );
